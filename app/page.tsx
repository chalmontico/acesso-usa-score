  ...
  return (
    <>
      ...
      <section style={{position:'relative',minHeight:'100vh',display:'flex',flexDirection:'column',justifyContent:'center',padding:'0 5vw',overflow:'hidden'}}>
        ...
        {/* FOTO DO AVIÃO DE FUNDO */}
        <div style={{position:'absolute',right:0,top:0,bottom:0,width:'60%',zIndex:0,overflow:'hidden'}}>
          <img
            src="/ACESSO-22.png"
            alt="Acesso USA"
            style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'center top'}}
          />
          <div style={{position:'absolute',inset:0,background:'linear-gradient(to right,rgba(6,8,16,1) 0%,rgba(6,8,16,0.8) 35%,rgba(6,8,16,0.3) 70%,rgba(6,8,16,0.5) 100%)',zIndex:1}}></div>
          <div style={{position:'absolute',inset:0,background:'rgba(6,8,16,0.35)',zIndex:2}}></div>
        </div>
      ...
      {/* FOTO EMPRESÁRIOS — seção Como funciona intro */}
      <section style={{position:'relative',height:'420px',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
        <img
          src="/ACESSO 2023-282.jpg"
          alt="Empresários Acesso USA"
          style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',objectPosition:'center'}}
        />
        <div style={{position:'absolute',inset:0,background:'rgba(6,8,16,0.75)'}}></div>
        <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(30,111,255,0.15),transparent)'}}></div>
        <div style={{position:'relative',zIndex:10,textAlign:'center',maxWidth:'700px',padding:'0 2rem'}}>
          <p style={{fontFamily:'Inter,sans-serif',fontSize:'clamp(1.4rem,3vw,2.4rem)',fontWeight:200,lineHeight:1.4,color:'#F0F4FF',letterSpacing:'-0.02em'}}>
            "Empresários brasileiros que já deram o primeiro passo para os <span style={{color:'#3FA9F5',fontWeight:600}}>Estados Unidos</span>"
          </p>
        </div>
      </section>
        ...
        <div style={{maxWidth:'1060px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'2px',background:'rgba(30,111,255,0.06)',border:'1px solid rgba(30,111,255,0.1)',borderRadius:'4px',overflow:'hidden'}}>
            {[
              ...
              <div key={n} style={{padding:'2.5rem 2rem',background:'#060810'}}>
      ...
      {/* FOTO ACESSO BLACK — antes da prova social */}
      <section style={{position:'relative',height:'380px',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
        <img
          src="/Acesso Black-21.png"
          alt="Acesso Black"
          style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',objectPosition:'center top'}}
        />
        <div style={{position:'absolute',inset:0,background:'rgba(6,8,16,0.72)'}}></div>
        <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(30,111,255,0.12),transparent)'}}></div>
        <div style={{position:'relative',zIndex:10,textAlign:'center',maxWidth:'700px',padding:'0 2rem'}}>
          <div className="section-tag" style={{justifyContent:'center',marginBottom:'1rem'}}>Acesso Black</div>
          <p style={{fontFamily:'Inter,sans-serif',fontSize:'clamp(1.3rem,3vw,2.2rem)',fontWeight:200,lineHeight:1.4,color:'#F0F4FF',letterSpacing:'-0.02em'}}>
            Uma comunidade de empresários brasileiros construindo presença nos <span style={{color:'#3FA9F5',fontWeight:600}}>Estados Unidos</span>
          </p>
        </div>
      </section>
      ...
      <section style={{padding:'7rem 5vw',background:'#111420',position:'relative',overflow:'hidden'}}>
        <div style={{maxWidth:'1060px',margin:'0 auto'}}>
          ...
          <h2 style={{fontFamily:'Inter,sans-serif',fontSize:'clamp(1.7rem,4vw,3.5rem)',fontWeight:200,lineHeight:1.15,marginBottom:'4rem',letterSpacing:'-0.025em'}}>
            Empresários que já <span style={{fontWeight:600,background:'linear-gradient(135deg,#0A3D91,#1E6FFF,#3FA9F5)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>fizeram o diagnóstico</span>
          </h2>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'2px',background:'rgba(30,111,255,0.08)',border:'1px solid rgba(30,111,255,0.1)',borderRadius:'4px',overflow:'hidden'}}>
            {[
              ['★★★★★','"Descobri que minha marca estava em risco nos EUA antes de qualquer investimento."','O diagnóstico foi cirúrgico. Saí com clareza total sobre o que precisava resolver primeiro.','Ricardo Almeida','CEO · Startup de Tecnologia · São Paulo'],
              ['★★★★★','"Finalmente entendi o que estava faltando na minha estratégia."','O roadmap gerado foi o começo real da minha expansão para os EUA. Clareza total.','Fernanda Costa','Fundadora · Moda Brasileira · Rio de Janeiro'],
              ['★★★★★','"Achei que estava pronto. O score mostrou 58/100 e mudou tudo."','A reunião estratégica virou completamente minha visão. Saí com um plano real.','Marcos Pereira','Diretor · Consultoria · Belo Horizonte'],
            ].map(([stars,quote,text,name,role]) => (
              <div key={name} style={{padding:'2.5rem',background:'#111420'}}>
                <div style={{color:'#3FA9F5',letterSpacing:'3px',fontSize:'0.72rem',marginBottom:'1rem',opacity:0.8}}>{stars}</div>
                <div style={{fontFamily:'Inter,sans-serif',fontSize:'1rem',fontWeight:300,color:'#F0F4FF',lineHeight:1.5,marginBottom:'1rem',letterSpacing:'-0.01em'}}>{quote}</div>
                <p style={{fontSize:'0.78rem',color:'rgba(240,244,255,0.7)',lineHeight:1.7,marginBottom:'1.4rem',fontStyle:'italic'}}>{text}</p>
                <div style={{fontSize:'0.72rem',fontWeight:600,letterSpacing:'0.1em',textTransform:'uppercase',color:'#F0F4FF'}}>{name}</div>
                <div style={{fontSize:'0.68rem',color:'rgba(240,244,255,0.35)',marginTop:'0.2rem'}}>{role}</div>
              </div>
            ))}
          </div>
        </div>
